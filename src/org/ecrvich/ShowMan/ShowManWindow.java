...
public class ShowManWindow extends Window implements Runnable
{
	...
	public ShowManWindow( Frame frame, Vector images,
			ShowManOptions options )
	{
		this( frame, images, null, options );
	}
	
	public ShowManWindow( Frame frame, Vector images, Vector filenames,
			ShowManOptions options )
	{
		super( frame );
		this.frame = frame;
	...
	private int showImages( int index )
	{
		...
		if (options.thumbs)
		{
			...
			for (int i = 0; i < cells; ++i)
				addCanvas( new ShowManCanvas( THUMB_WIDTH, THUMB_HEIGHT,
						(filenames != null) ? FONT_SIZE : 0, true ), (i == 0) );
			validate();
			String fname;
			while (cell < cells && index < images.size() && !finished)
			{
				fname = (filenames == null) ? null :
						(String)filenames.elementAt( index );
				if (updateCanvas( (ShowManCanvas)canvases.elementAt( cell ),
						((Image)images.elementAt( index )).getScaledInstance(
						THUMB_WIDTH, THUMB_HEIGHT, Image.SCALE_DEFAULT ),
						(fname != null) ? fname.substring(
						fname.lastIndexOf( File.separator ) + 1 ) : null ))
				{
					good_image = true;
					++cell;
					++index;
//					repaint();
				...
				else
				{
					...
					System.err.println( "The file '" + filenames.elementAt( index ) +
		...
		{
			String fname = (filenames == null) ? null :
					(String)filenames.elementAt( index );
	...
	private boolean updateCanvas( ShowManCanvas canvas, Image image,
			String filename )
	{
	...
	public void run()
	{
		...
		while (!finished)
		{
			...
			if ((next_index = showImages( image_index )) < 0)
				break;
			requestFocus();
			sleep( !options.nocycle && image_index == 0 &&
					next_index >= images.size() );
