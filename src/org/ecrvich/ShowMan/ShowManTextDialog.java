...
public class ShowManTextDialog
      extends Dialog
{
   private class ShowManTextField
         extends TextField
   {
               ...
               switch (key)
               {
                  ...
                  default:
                     if (!Character.isDigit( (char)key )
                           || (getText().length() >= 6 && getSelectionStart() == getSelectionEnd()))
                        e.consume();
                     break;
               }
            }
   ...
   public ShowManTextDialog( Frame frame, String title, String field_text, int maxlen,
         ShowManOptions options )
   {
